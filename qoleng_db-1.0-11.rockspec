...
description = {
  ...
  detailed = [[
    qoleng_db is a Lua library for managing database connections and operations,
    featuring an ORM, query builder, and native PostgreSQL support via LuaJIT FFI.
  ]],
  homepage = "https://github.com/winslygeorge/qoleng_db_manager",
  license = "MIT"
}
dependencies = {
...
build = {
  ...
  -- The key 'qoleng_db' here must match the target name in your CMakeLists.txt
  -- (e.g., `add_library(qoleng_db SHARED ...) `).
  ...
  install = {
    ...
    lua = {
      ...
      ["orm.QolengModelRoute"] = "build/orm/QolengModelRoute.lua",
      ...
      -- The native 'qoleng_db.so' is handled by the 'targets' section above and does not
      -- need to be listed here in 'install.lua'. If you have a Lua file that serves
      -- as the main `qoleng_db` module wrapper, that .luac file would be listed here.
    },
