...
description = {
   ...
   detailed = [[
      ...
      autonomous system number (ASN), autonomous system (AS), AS domain, AS usage type and AS CIDR
      from IP address by using IP2Location database.
      ...
      time zone, ISP, domain name, connection type, IDD code, area code, weather station code, station name,
      mcc, mnc, mobile brand, elevation, usage type, address type, IAB category, district,
      autonomous system number (ASN), autonomous system (AS), AS domain, AS usage type and AS CIDR as values.
