...
-- layered `config.toml` at activation and workflow profiles afterwards, so it
...
description = {
    ...
    detailed = [[
        Lets a primary panto agent start specialized child agents from subagent
        ...
        one-shot structured workers, and workflow profiles (TOML dependency graphs)
        exposed as /workflow:<name> commands.
    ]],
